...
static QString getWindowExeDir(HWND window)
{
    DWORD dwProcessId;
    ...
    return QString::fromStdWString(std::filesystem::path(path).parent_path().wstring());
}
...
QString getFrontWindowExeDir()
{
    ...
    if (!atExplorer1 && !atExplorer2 && !atDesktop1 && !atDesktop2)
        return getWindowExeDir(focusedWindow);
    ...
    if (atDesktop1 || atDesktop2)
    {
        ...
        return QString::fromStdWString(std::wstring(path));
    }
